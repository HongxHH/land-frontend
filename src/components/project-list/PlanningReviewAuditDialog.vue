    ...
    <div ref="auditLayoutRef" class="audit-split-layout audit-split-layout--responsive planning-audit-shell">
      <section class="audit-split-layout__left audit-preview-shell" :style="leftPanelStyle">
        ...
        >
          ...
          <template #preprocess>
            <iframe v-if="pdfUrl" :src="pdfUrl" class="audit-preview-iframe" title="预处理文件预览" />
            <div v-else class="audit-preview-empty">
      ...
      <section class="rows-panel audit-split-layout__right">
        ...
        <div ref="rowsTableWrapRef" class="rows-table-wrap" v-loading="rowsLoading">
          ...
          >
            ...
            <el-table-column prop="engineeringProject" label="工程项目/楼栋" min-width="160" fixed="left" show-overflow-tooltip>
...
const { auditLayoutRef, leftPanelStyle, onSplitterMouseDown } = useAuditSplitPanel({
  onSplitEnd: () => nextTick(measureTableHeight)
})
...
function bindTableHeightObserver() {
  ...
  if (auditLayoutRef.value) tableResizeObserver.observe(auditLayoutRef.value)
}
